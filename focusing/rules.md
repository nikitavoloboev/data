...
- For domains, get literal name, avoid dashes. Keep it to one registrar (Google Domains preferred). Prefer `.com` or `.co` generically. If it's an app, use `.app`. Developer related, `.dev` is good. Organization, then `.org.`. Otherwise `.io`, `.so`, `.xyz` are good. `.xyz` is nice for educational content. Can also play on domain name like [lobste.rs](https://lobste.rs/) if possible. Make domain name memorable.
