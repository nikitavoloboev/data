...
- Twitter/Reddit/Instagram/GitHub, get literal name if available. On Twitter, [lowercasing](https://twitter.com/webkit) is good if name is one word (Twitter will show your custom real name in searches already). If name includes multiple words [capitalizing](https://twitter.com/khanacademyeng) can look better for recognition. Capitalize [Reddit](https://www.reddit.com/r/ApolloApp/) subreddit name as it stands out better on searches (the name can't be changed later so capitalize even if one word). GitHub is good [lowercased](https://github.com/learn-anything), literal name or with dashes. If name is taken, can add \_ or [HQ](https://twitter.com/SlackHQ)/[App](https://twitter.com/SparkMailApp) in the end. For Instagram, if literal name is taken, it's possible to get domain name literal like [narative.co](https://www.instagram.com/narative.co/). Can use [namecheckr](https://www.namecheckr.com/) to check for available names.
- For domains, get literal name, avoid dashes. Keep it to one registrar (Google Domains preferred). Prefer `.com`, ([vercel.com](https://vercel.com/)), `.co` ([vsco.co](https://vsco.co/)) or `.io` ([codesandbox.io](https://codesandbox.io/)) for generic sites. If it's an app, can use `.app` ([linear.app](https://linear.app)), can also have app in name like [apolloapp.io](https://apolloapp.io/) if `.app` is taken. Developer related, `.dev` ([web.dev](https://web.dev)) is good. Organization, then `.org.` ([telegram.org](https://telegram.org/)). Otherwise, `.so`, ([notion.so](https://www.notion.so/))`.xyz` are good. `.xyz` is nice for educational content ([learn-anything.xyz](https://learn-anything.xyz)). Can also play on domain name like [lobste.rs](https://lobste.rs/) if possible. Make domain name memorable.
