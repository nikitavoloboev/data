...
- Twitter/Reddit/Instagram/GitHub, get literal name if available. On Twitter, [lowercasing](https://twitter.com/webkit) is good if one word, otherwise if multiple words [capitalizing](https://twitter.com/GoogleAI) is better for recognition. Capitalize [Reddit](https://www.reddit.com/r/ApolloApp/) name as it stands out better on searches. GitHub [lowercased](https://github.com/learn-anything), literal name or with dashes. If name taken, can add \_ or [HQ](https://twitter.com/SlackHQ)/[App](https://twitter.com/SparkMailApp) in the end. For Instagram, if literal name is taken, it's possible to get domain name literal like [narative.co](https://www.instagram.com/narative.co/). Can use [namecheckr](https://www.namecheckr.com/) to check for available names.
- For domains, get literal name, avoid dashes. Keep it to one registrar (Google Domains preferred). Prefer `.com`, `.co` or `.io` if not too expensive and not taken. If it's primarily an app, use `.app`. Otherwise `.so`, `.dev`, `.org`, `.xyz` are good. Can also play on domain name like [lobste.rs](https://lobste.rs/) if possible.
