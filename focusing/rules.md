...
- For domains, get literal name, avoid dashes. Keep it to one registrar (Google Domains preferred). Prefer `.com` or `.co` if not too expensive and not taken. If it's an app, use `.app`. Otherwise `.io`, `.so`, `.dev`, `.org`, `.xyz` are good. Can also play on domain name like [lobste.rs](https://lobste.rs/) if possible. Make domain name memorable.
